...
var setChain = function (networkConfigPath, configPath) {
...
var buildTarget = function (peer, org) {
	...
	if (typeof peer !== 'undefined') {
		let targets = newPeers([getPeerAddressByName(org, peer)]);
...
var enrollUser = function (username, enrollmentSecret, csr, userOrg) {
	return caClients[userOrg].enroll({
		enrollmentID: username,
...
var revokeUser = function (username, certificate, reason, userOrg) {
	...
	return hfc.newDefaultKeyValueStore({
		...
		return client.getUserContext(username, true).then((user) => {
				...
				return getAdminUser(userOrg).then(function (adminUserObj) {
					member = adminUserObj;
					return caClients[userOrg].revoke({
						enrollmentID: username,
						reason:reason
					},member);
				}).then((response) => {
					logger.debug(response.toString() + ' Revoked successfully.');
					return response;
				}, (err) => {
					logger.debug(username + ' Revocation failed.');
					throw new Error(err);
					//return 'Failed to register '+username+'. Error: ' + err.stack ? err.stack : err;
				});
		});
...
module.exports = {
	getConfig: getConfig,
	getChannelForOrg: getChannelForOrg,
	getClientForOrg: getClientForOrg,
	getLogger: getLogger,
	setupChaincodeDeploy: setupChaincodeDeploy,
	getMspID: getMspID,
	newPeers: newPeers,
	newEventHubs: newEventHubs,
	getPeerAddressByName: getPeerAddressByName,
